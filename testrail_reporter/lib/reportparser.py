...
class ReportParser(object):
    ...
    def get_result_list(self, xml_file):
        ...
        for res in self.raw_results:
            if 'filter_setup' in self.tr_result_attrs:
                pattern = self.tr_result_attrs['filter_setup']['match']
                actions = self.tr_result_attrs['filter_setup']['actions']
            ...
            if 'filter_teardown' in self.tr_result_attrs:
                pattern = self.tr_result_attrs['filter_teardown']['match']
                actions = self.tr_result_attrs['filter_teardown']['actions']
                if re.match(pattern, res['test_id']):
                    out = perform_actions(res['test_id'],
                                          actions)
                    res['test_id'] = out
