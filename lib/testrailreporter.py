...
class TestRailReporter:
    ...
    def update_test_suite(self, name, tc_list):
        ...
        for tc in tc_list:
            ...
            if isinstance(tc['section_id'], str):
                try:
                    self.convert_section2id(tc, suite)
                except Exception:
                    section_data = {"name": tc['section_id'], 'suite_id': suite_id}
                    self.project.add_section_project(section_data)
                    self.convert_section2id(tc, suite)

            if tc['section_id'] not in tr_cases:
                tr_cases[tc['section_id']] = self.project.get_cases_project(
                    suite_id, tc['section_id'])
                tr_cases[tc['section_id']]['titles'] = []
                for tr_tc in tr_cases[tc['section_id']]['titles']:
                    tc['section_id']['titles'].append(tr_tc['title'])

            if tc['title'] not in tc['section_id']['titles']:
                self.project.add_case(tc['section_id'], tc)
    ...
    def convert_section2id(self, test_case, suite):
        sections = self.project.get_sections_project(suite['id'])
        for i in sections:
            if test_case['section_id'] == i['name']:
                test_case['section_id'] = i['id']
                return
        raise Exception("Can't find Section '{}' in Test Suite '{}'"
                        "".format(test_case['section_id'], suite['name']))
