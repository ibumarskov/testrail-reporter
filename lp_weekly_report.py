...
milestone = launchpad.projects[project].getMilestone(name=milestone)
list_of_bugs = milestone.searchTasks()
tbl_qa = copy.deepcopy(table)
tbl_other = copy.deepcopy(table)

for bug in list_of_bugs:
    tbl_other.add_row([bug.title[25:125],
                       trim_milestone(bug.milestone),
                       bug.importance,
                       bug.status,
                       bug.bug.owner.display_name,
                       bug.assignee.display_name,
                       bug.web_link])
print "\n\n\nList of bugs\n"
print tbl_other.get_string(sortby="Reporter")
print "Total bugs need to be verified: {0}".format(len(list_of_bugs))
